    ...
    (set, get) => {
      ...
      return {
        ...
        generateImage: async () => {
          ...
          const activeLayerId = layerStoreState.activeLayerId;
          const activeLayer = layerStoreState.layers.find(
            (l) => l.id === activeLayerId
          );
          const wasLocked = activeLayer ? activeLayer.locked : false;
          if (activeLayer && !wasLocked) {
            layerStoreState.toggleLock(activeLayerId);
          }
          ...
          try {
            ...
            const executeDrawPixel = async (
              ...
              if (pixelData.length > 0) {
                setLayerPixels(activeLayerId, pixelData, true); // Force update even if locked
                set((state) => ({
            ...
            if (currentLayer && !wasLocked && currentLayer.locked) {
              useLayerStore.getState().toggleLock(activeLayerId);
            }
